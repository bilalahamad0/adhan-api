...
async function runTest() {
  ...
  const playingSession = sessionOutput
    .split('\n')
    .filter(
      (l) =>
        l.includes('state=3') ||
        l.includes('state=Playing') ||
        l.includes('isActive=true') ||
  ...
  const playingAudio = audioOutput
    .split('\n')
    .filter(
      (l) =>
        l.includes('state:started') ||
        l.includes('playerState=2') ||
        l.includes('usage=USAGE_MEDIA')
    );
