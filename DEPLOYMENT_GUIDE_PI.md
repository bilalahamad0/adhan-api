...
This guide will help you migrate and host the `adhan-api/audio-caster` on your Raspberry Pi 4 (IP: `10.0.0.110`).

## 1. Prerequisites (Run on Raspberry Pi)

First, SSH into your Raspberry Pi:
```bash
ssh <your-user>@10.0.0.110
```
...
scp -r /Users/bilalahamad/Documents/GitHub/adhan-api <pi-user>@10.0.0.110:~/adhan-api
```
...
HOST_IP=10.0.0.110         <-- IMPORTANT: Set this to your Pi's IP
SERVER_PORT=3001

# Target Devices
TV_IP=10.0.0.80            <-- Your Android TV IP
...
2.  Run:
    ...
    adb connect <TV_IP>
    ```
    *(Replace `<TV_IP>` with your TV's actual IP, e.g., 10.0.0.80)*
...
4.  Verify connection:
    ```bash
    adb devices
    # Should show: 10.0.0.80:5555 device
    ```
...
> `🔊 Local Audio Server running at http://10.0.0.110:3001/audio/`

## Troubleshooting

-   **Wrong IP**: If the cast URL shows `127.0.0.1` or wrong IP, ensure `HOST_IP=10.0.0.110` is set in `.env`.
...
-   **TV not pausing / ADB Error**:
    -   If logs say `adb: device unauthorized`:
        ...
        2.  On Pi: `adb kill-server` then `adb connect 10.0.0.80`.
