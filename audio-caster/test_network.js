...
exec(`ping -c 1 -W 2 ${TV_IP}`, (err, stdout, stderr) => {
    if (err) {
        ...
        console.log(`   -> ACTION: Check if TV is on. Check if TV IP Address changed in Network Settings.`);
        return;
    }
...
function checkPort(host, port) {
    ...
    socket.on('timeout', () => {
        ...
        console.log(`   -> ACTION: Go to TV Settings > Device Preferences > Developer Options > Enable USB Debugging.`);
        socket.destroy();
    });
