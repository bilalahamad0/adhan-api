...
async function runScenarios() {
    ...
    const buf1 = await visualGen.generateDashboard("Dhuhr", time, hijri, { holidays: [], isFriday: false });
    ...
    const buf2 = await visualGen.generateDashboard("Dhuhr", time, hijri, { holidays: [], isFriday: true });
    ...
    const buf3 = await visualGen.generateDashboard("Dhuhr", time, hijri, { holidays: ['Eid-ul-Fitr'], isFriday: false });
    fs.writeFileSync(path.join(outputDir, 'test_eid.jpg'), buf3);
    console.log("✅ Saved test_eid.jpg");

    // Scenario 4: Isha (Random Isha)
    console.log("\n--- Scenario 4: Isha ---");
    const buf4 = await visualGen.generateDashboard("Isha", "8:00 PM", hijri, { holidays: [], isFriday: false }); // Corrected isFriday to false for standard Isha test
